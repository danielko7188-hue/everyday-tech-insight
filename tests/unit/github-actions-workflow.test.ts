...
describe("owner-only repository controls", () => {
  ...
  it("rejects every pull request not authored by the sole owner without executing pull-request code", () => {
    ...
    expect(steps).toEqual([
      {
        ...
        env: {
          ...
          PUBLISHING_OWNER_ID: "239253033",
        },
        run: [
          'if [ "$PR_AUTHOR_ID" != "$PUBLISHING_OWNER_ID" ]; then',
          '  echo "Only the publishing owner account may author a pull request."',
          "  exit 1",
          "fi",
    ...
    expect(readFileSync(workflowPath, "utf8")).not.toMatch(
      /checkout|pull_request\.head|pull_request\.user\.login|github\.head_ref|github\.token|GITHUB_EVENT_PATH|secrets\.|\b(?:curl|wget|git|gh)\b/i,
    );
  });
});
