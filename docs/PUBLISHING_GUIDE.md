...
5. Reconfirm the live GitHub controls before release. At the start of the owner-only-control release on 2026-08-26, `main` required the app-bound `Full quality gate`, strict checks, admin enforcement, linear history, and conversation resolution; force pushes and deletion were disabled, and no bypass actors were configured. The candidate `Owner-only publishing gate` cannot run from a feature branch because GitHub loads this security workflow from trusted `main`. Merge the foundation through the existing gate, exercise the owner-only workflow from the updated base, add its app-bound context to required checks, and verify both contexts before routine publishing. Repository Actions permit GitHub-owned actions only, and every external fork contributor requires workflow approval. `content/editorial` permits direct CMS saves while enforcing admins, linear history, and no force pushes or deletion. Hosted settings can change. This solo-owner setup requires no second-person approval; record the owner's actual review rather than inventing a reviewer.
...
4. Confirm the pull request was authored by `danielko7188-hue`; then run the local checks below and require both `Owner-only publishing gate` and `Full quality gate`. The owner-only check must run from the trusted base branch without checking out pull-request code or receiving token permissions. The one-time foundation PR that first adds this workflow can run only the pre-existing full-quality check; immediately after that merge, activate and prove the owner-only required context on a second owner-authored PR before any routine publication.
