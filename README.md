...
At the start of this release, protected `main` requires pull requests and the strict app-bound `Full quality gate`, with admin enforcement, linear history, conversation resolution, no bypass actors, and force pushes and deletion disabled. This release candidate adds `Owner-only publishing gate`, but that trusted-base workflow cannot run until it is merged to `main`; it must then be exercised and added to required branch protection before it is described as active. The candidate uses no token permissions, does not check out pull-request code, and authorizes only immutable GitHub user ID `239253033`, currently `danielko7188-hue`, so an account rename cannot lock out the owner. `.github/CODEOWNERS` assigns every path to that same sole owner. Required CODEOWNER approval is intentionally not enabled because a solo pull-request author cannot approve their own pull request.
