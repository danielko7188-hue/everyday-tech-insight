...
Implemented surfaces include a story-led homepage with coordinated editorial covers, CSS-only reading progress on article pages, native “At a glance” disclosures, a real-data Toolkit structure preview on each Toolkit detail route, and direct 404 recovery links, including Toolkit. The unrelated homepage glass artwork and unused category/topic motifs are removed from those pages; decorative 404 motifs remain hidden. Required release-review widths are 320, 390, 600, 768, 1024, 1280, 1440, and 1920px.

## Current publication boundary

The September 9, 2026 story-led editorial redesign supersedes the first Apple-inspired presentation pass. Self-hosted Instrument Sans variable typography, a neutral-blue palette, compact introductions, a dominant lead story and subject-specific studio illustrations define the current direction. Covers are explicitly labeled AI-generated conceptual illustrations, not real product tests or reporting evidence; originals, prompts and delivery hashes are retained. The licensed local Source Sans font remains available for explanatory diagrams and deterministic social-image generation. This is an independent publication, not an Apple clone or affiliation. The nine-guide homepage, informative article diagrams, owner-only publishing controls, and monetization-off state remain intact.

See the [current design contract](docs/superpowers/specs/2026-09-09-editorial-art-direction-design.md), [cover provenance](docs/EDITORIAL_COVERS_2026-09-09.md), [historical decorative artwork provenance](docs/EDITORIAL_ART_PROVENANCE_2026-09-09.md), and [AdSense review](docs/ADSENSE_REVIEW_2026-09-09.md). The unmodified Instrument Sans copyright notice and OFL-1.1 license are distributed at `/fonts/LICENSE-instrument-sans.txt`. Four articles received substantive clarity or worked-example updates in the earlier September release. Fresh release checks and exact-SHA deployment evidence are required for this revision; earlier observations do not certify it.
...
The exact repository is `danielko7188-hue/everyday-tech-insight`; the release branch is `main`; and the owner editorial branch is `content/editorial`. The remote editorial branch and `main` protection were verified on GitHub on 2026-08-26. Live GitHub and Vercel API checks on the same date found one direct GitHub collaborator—the owner—with no pending repository invitations, deploy keys, or repository webhooks, and one Vercel team member—the owner—with no Vercel webhooks. These are dated observations, not proof about installed GitHub Apps or hosted Pages CMS accounts.

After foundation [PR #2](https://github.com/danielko7188-hue/everyday-tech-insight/pull/2), a live GitHub API inspection on 2026-08-26 confirmed that protected `main` requires pull requests and both strict app-bound checks: `Owner-only publishing gate` and `Full quality gate`. Admin enforcement, linear history, and conversation resolution remain active; no bypass actors are configured; and force pushes and deletion remain disabled. The trusted-base owner gate was exercised successfully on owner-authored [PR #3](https://github.com/danielko7188-hue/everyday-tech-insight/pull/3) ([check run](https://github.com/danielko7188-hue/everyday-tech-insight/actions/runs/33019254652/job/98345242404)). It uses no token permissions, does not check out pull-request code, and authorizes only immutable GitHub user ID `239253033`, currently `danielko7188-hue`, so an account rename cannot lock out the owner. `.github/CODEOWNERS` assigns every path to that same sole owner. Required CODEOWNER approval is intentionally not enabled because a solo pull-request author cannot approve their own pull request.

Repository Actions permit GitHub-owned actions only, and workflows from every external fork contributor require approval. `content/editorial` permits the CMS's direct saves but enforces admins, linear history, and no force pushes or deletion. The owner should grant the Pages CMS GitHub App access to this exact repository only and must not invite Pages CMS collaborators. Hosted collaborator absence, exact GitHub App scope, hosted sign-in, Pages CMS branch selection, media upload, and save/commit round-trip all remain unverified.
...
The visual suite selects its committed platform-specific references automatically. The repository retains 37 reviewed states for Windows and Linux, including Toolkit full-page coverage at 390, 600, 768, 1024, and 1440px, for 74 platform-specific baseline files. Update either platform set only after examining the rendered change; the comparison tolerance remains unchanged.
...
- `.github/CODEOWNERS` and `.github/workflows/owner-only.yml`: sole-owner path attribution and the trusted-base, no-checkout pull-request author gate. The workflow file alone is not enforcement; live required-check protection must be activated and rechecked separately.
...
Category pages adapt to content volume: fewer than six guides use one compact list with an image-led first story, six to eleven use a lead and supporting sequence, and twelve or more use a three-guide priority opening, the next three recent guides, and the remaining archive. Every published category guide remains linked exactly once. Ordering is deterministic: published featured guides first, then the newest `dateModified` or `datePublished`, then title and slug. The first compact-list story's visual prominence does not change its editorial status or the underlying order. Article illustrations are informative, story-specific local SVGs with text alternatives; separate browsing covers remain decorative with visible provenance. Toolkit field guidance is stacked on mobile, with no horizontally scrolling primary field table.
